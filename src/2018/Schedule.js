...
const Talk = ({ time, title, speaker, description, isSavable = true }) => (
  <StyledTalk>
    ...
    <div>
      ...
      {isSavable && <SaveButton name={title} />}
    </div>
  </StyledTalk>
);
...
class Schedule extends React.Component {
  ...
  getSchedule = () => (this.state.day === 1 ? day1 : day2);
        ...
        <SectionWrap id={this.props.id}>
          ...
          <Container>
            <CurrentDay>
              <Title>
                Day {this.state.day}{' '}
                <Switch onClick={this.switchDay}>
                  (view day {this.state.day === 1 ? '2' : '1'})
                </Switch>
              </Title>
              <p>
                {this.state.day === 1
            ...
            <ul>
              {this.getSchedule().map((item, i) => <Talk key={i} {...item} />)}
            </ul>
          </Container>
        </SectionWrap>
      </LocalSync>
